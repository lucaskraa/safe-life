...
(function () {
    ...
    const API_V18 = "https://safe-life.onrender.com";
    const TOKEN_KEY_V18 = "safeLifeAuthToken";
    const USER_KEY_V18 = "safeLifeLoggedUser";

    const FIXED_PHOTOS_V18 = {
        "11111111111": 'img/pequenochinique.jpeg',
        "99999999999": 'img/corredorzeca.jpeg',
        "45317828791": 'img/apenasumsiri.jpeg'
    };

    const FALLBACK_USER_V18 =
        "data:image/svg+xml;charset=UTF-8," +
        encodeURIComponent(
            '<svg xmlns="http://www.w3.org/2000/svg" width="240" height="240" viewBox="0 0 240 240">' +
            '<rect width="240" height="240" rx="120" fill="#eef2ff"/>' +
            '<circle cx="120" cy="84" r="42" fill="#94a3b8"/>' +
            '<path d="M39 220c9-55 39-82 81-82s72 27 81 82" fill="#94a3b8"/>' +
            '<text x="188" y="61" text-anchor="middle" font-size="30">🐾</text>' +
            '</svg>'
        );

    const FALLBACK_PET_V18 =
        "https://images.unsplash.com/photo-1543466835-00a7907e9de1?auto=format&fit=crop&w=900&q=80";

    let queueCacheV18 = null;
    let queueCacheAtV18 = 0;
    let missingPetsCacheV18 = [];
    let sessionTimerV18 = null;

    function byIdV18(id) {
        return document.getElementById(id);
    }

    function valueV18(id) {
        const element = byIdV18(id);
        return element ? String(element.value || "").trim() : "";
    }

    function cleanCpfV18(value) {
        return String(value || "").replace(/\D/g, "");
    }

    function escapeV18(value) {
    ...
    function applyPhoneMasksV18() {
        ...
        ids.forEach(function (id) {
            const field = byIdV18(id);
            if (!field || field.dataset.phoneMaskV18 === "true") return;

            field.dataset.phoneMaskV18 = "true";
            field.inputMode = "tel";
            field.maxLength = 15;
            field.addEventListener("input", function () {
                const cursorAtEnd = field.selectionStart === field.value.length;
                field.value = formatPhoneV18(field.value);
                if (cursorAtEnd) field.setSelectionRange(field.value.length, field.value.length);
            });
            field.value = formatPhoneV18(field.value);
        });
    }

    function currentUserV18() {
        try {
            if (window.usuarioLogado && window.usuarioLogado.cpf) {
        ...
        try {
            return JSON.parse(localStorage.getItem(USER_KEY_V18) || "null");
        } catch (_) {
            return null;
        }
    }

    function saveUserV18(user, token) {
        if (!user) return null;
        const cpf = cleanCpfV18(user.cpf);
        const photo = FIXED_PHOTOS_V18[cpf] || user.foto || user.foto_perfil || user.avatar || "";
        const normalized = {
            ...user,
            cpf,
            foto: photo,
        ...
        localStorage.setItem(USER_KEY_V18, JSON.stringify(normalized));
        if (token) localStorage.setItem(TOKEN_KEY_V18, token);
        window.usuarioLogado = normalized;
        return normalized;
    }

    function clearSessionV18() {
        localStorage.removeItem(TOKEN_KEY_V18);
        localStorage.removeItem(USER_KEY_V18);
        localStorage.removeItem("safeLifeLastCpf");
        window.usuarioLogado = null;
    }

    function toastV18(message) {
        if (typeof window.triggerToast === "function") {
            window.triggerToast(message);
            return;
        }

        const toast = byIdV18("toast");
    ...
    function showScreenV18(id) {
        ...
        const screen = byIdV18(id);
        if (screen) screen.classList.add("active");
    }

    function accountModalV18(code, message, blockedUntil) {
        const modal = byIdV18("accountStatusModal");
        const title = byIdV18("accountStatusTitle");
        const text = byIdV18("accountStatusMessage");
        const icon = byIdV18("accountStatusIcon");

        if (!modal) {
            alert(message);
            clearSessionV18();
            showScreenV18("loginScreen");
            return;
        }

        if (code === "ACCOUNT_DELETED") {
            if (title) title.textContent = "Conta excluída";
            if (icon) icon.textContent = "🗑️";
        } else if (code === "ACCOUNT_SUSPENDED") {
            if (title) title.textContent = "Conta suspensa";
            if (icon) icon.textContent = "⏳";
        } else {
        ...
        let finalMessage = message || "Sua sessão não está mais disponível.";
        if (blockedUntil) {
            const date = new Date(blockedUntil);
            if (!Number.isNaN(date.getTime())) {
                finalMessage += `\n\nSuspensão válida até ${date.toLocaleString("pt-BR")}.`;
            }
        }
    ...
    window.encerrarSessaoPorStatus = function () {
        clearSessionV18();
        const modal = byIdV18("accountStatusModal");
    ...
    async function apiV18(endpoint, options = {}, config = {}) {
        const headers = new Headers(options.headers || {});
        if (!headers.has("Content-Type")) headers.set("Content-Type", "application/json");

        const token = localStorage.getItem(TOKEN_KEY_V18) || "";
        if (token && !headers.has("Authorization")) {
            headers.set("Authorization", "Bearer " + token);
        }

        const response = await fetch(API_V18 + endpoint, {
            ...options,
            headers
        });
        ...
        if (!response.ok) {
            ...
            error.blockedUntil = data && data.blockedUntil;
            error.data = data;

            if (
                !config.ignoreAccountState &&
                ["ACCOUNT_DELETED", "ACCOUNT_SUSPENDED", "SESSION_REVOKED"].includes(error.code)
            ) {
                accountModalV18(error.code, error.message, error.blockedUntil);
            }
    ...
    window.safeLifeApiV18 = apiV18;

    async function fileToDataUrlV18(id) {
        const input = byIdV18(id);
    ...
    function setBusyV18(button, busy, text) {
        if (!button) return;
        if (busy) {
            button.dataset.oldTextV18 = button.textContent;
            button.disabled = true;
            button.textContent = text || "Aguarde...";
        } else {
            button.disabled = false;
            if (button.dataset.oldTextV18) button.textContent = button.dataset.oldTextV18;
        }
    }

    function protectImageV18(image, fallback) {
        if (!image) return;
        image.onerror = function () {
            image.onerror = null;
            image.src = fallback || FALLBACK_USER_V18;
        };
    }

    function forceProfilePhotosV18() {
        const user = saveUserV18(currentUserV18());
        const cpf = cleanCpfV18(user && user.cpf);
        const map = {
            profileAvatar: FIXED_PHOTOS_V18[cpf] || (user && user.foto) || FALLBACK_USER_V18,
            proAvatar: FIXED_PHOTOS_V18["99999999999"],
            professionalProfileAvatar: FIXED_PHOTOS_V18["99999999999"],
            adminAvatar: FIXED_PHOTOS_V18["45317828791"],
            adminProfileAvatar: FIXED_PHOTOS_V18["45317828791"]
        };

        Object.entries(map).forEach(function ([id, source]) {
            const image = byIdV18(id);
            if (!image || !source) return;
            if (image.src !== source) image.src = source;
            protectImageV18(image, FALLBACK_USER_V18);
        });
    }

    function userPhotoV18(item) {
        const cpf = cleanCpfV18(item.cpf_usuario || item.cpf || item.cpf_dono || "");
        if (FIXED_PHOTOS_V18[cpf]) return FIXED_PHOTOS_V18[cpf];

        const source = String(
            item.foto_usuario || item.foto_dono || item.foto_perfil || item.avatar || ""
        ).trim();

        if (/^(data:image\/|https?:\/\/)/i.test(source)) return source;
        return FALLBACK_USER_V18;
    }

    function petPhotoV18(item) {
        const source = String(item.foto || "").trim();
        if (/^(data:image\/|https?:\/\/)/i.test(source)) return source;
        return FALLBACK_PET_V18;
    }

    function routeUserV18(user) {
        if (!user) return;
        if (user.type === "admin" || user.cpf === "45317828791") {
            ...
            showScreenV18("menuScreen");
        }
    }

    window.autenticar = async function () {
        const button = document.querySelector('button[onclick*="autenticar"]');
        const cpf = cleanCpfV18(valueV18("cpfInput"));
        const senha = valueV18("loginPassword");
        const role = valueV18("loginRole") || "citizen";
        const company = valueV18("loginCompany");

        if (cpf.length !== 11 || !senha) {
            alert("Digite o CPF com 11 números e a senha.");
            return;
        }

        try {
            setBusyV18(button, true, "Entrando...");
            const response = await apiV18("/api/auth/login", {
                method: "POST",
                body: JSON.stringify({ cpf, senha, role, company })
            }, { ignoreAccountState: true });

            const user = saveUserV18(response.user, response.token);
            toastV18("✅ Login realizado.");
            routeUserV18(user);
        } catch (error) {
            if (["ACCOUNT_DELETED", "ACCOUNT_SUSPENDED"].includes(error.code)) {
                accountModalV18(error.code, error.message, error.blockedUntil);
            } else {
                alert(error.message);
            }
        } finally {
            setBusyV18(button, false);
        }
    };

    window.efetuarCadastro = async function () {
        const button = document.querySelector('button[onclick*="efetuarCadastro"]');
        const nome = valueV18("regName");
        const cpf = cleanCpfV18(valueV18("regCpf"));
        const email = valueV18("regEmail");
        const telefone = formatPhoneV18(valueV18("regPhone"));
        const senha = valueV18("regPassword");
        const confirm = valueV18("regPasswordConfirm");
        const type = valueV18("regType") || "citizen";
        const company = valueV18("regCompany");

        if (!nome || cpf.length !== 11 || !email || !telefone || !senha) {
            alert("Preencha todos os campos obrigatórios.");
            return;
        }
        if (senha.length < 6 || senha !== confirm) {
            alert("As senhas precisam ser iguais e ter pelo menos 6 caracteres.");
            return;
        }

        try {
            setBusyV18(button, true, "Criando cadastro...");
            const photo = await fileToDataUrlV18("regAvatarFile").catch(function () { return ""; });
            const response = await apiV18("/api/auth/register", {
                method: "POST",
                body: JSON.stringify({ nome, cpf, email, telefone, senha, type, company, foto: photo })
            });
            const user = saveUserV18(response.user, response.token);
            toastV18("✅ Conta criada.");
            routeUserV18(user);
        } catch (error) {
            alert(error.message);
        } finally {
    ...
    async function checkSessionV18() {
        const token = localStorage.getItem(TOKEN_KEY_V18);
        if (!token) return;

        try {
            const response = await apiV18("/api/auth/session-status", {}, { ignoreAccountState: true });
            if (response && response.user) saveUserV18(response.user);
        } catch (error) {
            if (["ACCOUNT_DELETED", "ACCOUNT_SUSPENDED", "SESSION_REVOKED", "SESSION_INVALID"].includes(error.code)) {
                accountModalV18(error.code, error.message, error.blockedUntil);
            }
        }
    }

    function isDemoCallV18(item) {
        const subject = String(item.assunto || "").toLowerCase();
        const location = String(item.localizacao || "").toLowerCase();
        const photo = String(item.foto || "");
        return (
            (subject === "animal na rua" && location.startsWith("rua das flores")) ||
            (subject === "animal ferido" && location.startsWith("avenida principal")) ||
            (subject === "sem água e comida" && location.startsWith("rua esperança")) ||
            (subject === "animal acorrentado" && location.startsWith("travessa das palmeiras")) ||
            /photo-1558788353|photo-1574158622682|photo-1583512603805|photo-1596492784531/.test(photo)
        );
    }

    async function fetchQueueV18(force = false) {
        const now = Date.now();
        if (!force && queueCacheV18 && now - queueCacheAtV18 < 5000) return queueCacheV18;

        const data = await apiV18("/api/pro/ocorrencias");
        queueCacheV18 = (Array.isArray(data) ? data : []).filter(function (item) {
            return !isDemoCallV18(item) && !["CONCLUIDA", "CANCELADA"].includes(String(item.status || "").toUpperCase());
        });
        queueCacheAtV18 = now;
        return queueCacheV18;
    }

    function callCardV18(item, compact = false) {
        const origin = String(item.origem || "ocorrencia");
        const id = Number(item.id);
        const anonymous = origin === "anonima" || item.anonima === true;
        const reporter = anonymous ? "Denúncia Anônima" : item.nome_usuario || "Cidadão";
        const avatar = anonymous
            ? '<div class="anon-avatar">🕶️</div>'
            : `<img class="reporter-avatar" src="${escapeV18(userPhotoV18(item))}" alt="Foto de ${escapeV18(reporter)}">`;
        const evidence = item.foto
            ? `<img class="evidence-image" src="${escapeV18(item.foto)}" alt="Evidência enviada">`
            : '<div class="evidence-empty">Nenhuma foto enviada.</div>';
        const detailsId = `call-details-v18-${origin}-${id}`;

        return `
            <article class="prof-occurrence-card ${anonymous ? "anon" : ""}" data-origin="${escapeV18(origin)}" data-id="${id}">
                <div class="prof-occurrence-top">
                    ${avatar}
                    <div class="prof-occurrence-meta">
                        <h4>${escapeV18(item.opcao_escolhida || item.assunto || item.tipo || "Chamado")}</h4>
                        <p><strong>Nome:</strong> ${escapeV18(reporter)}</p>
                        <small>${escapeV18(item.tipo || "Chamado")} • ${escapeV18(item.status || "PENDENTE")}</small>
                ...
                <div class="prof-occurrence-body">
                    <div class="prof-line"><strong>Endereço atual:</strong><br>${escapeV18(item.endereco_completo || item.localizacao || "Não informado")}</div>
                    <div class="prof-line"><strong>Descrição:</strong><br>${escapeV18(item.detalhes || "Sem descrição")}</div>
                    ${evidence}
                    <div class="prof-details ${compact ? "" : "show"}" id="${detailsId}">
                        <div class="prof-line"><strong>Prioridade:</strong> ${escapeV18(item.prioridade || "NORMAL")}</div>
                    </div>
                    <div class="prof-actions-inline">
                        ${compact ? `<button class="btn secondary-btn" type="button" onclick="document.getElementById('${detailsId}').classList.toggle('show')">Ver detalhes</button>` : ""}
                        <button class="btn" type="button" onclick="atualizarChamadoV18('${escapeV18(origin)}', ${id}, 'EM_ATENDIMENTO', this)">Em atendimento</button>
                        <button class="btn" type="button" onclick="atualizarChamadoV18('${escapeV18(origin)}', ${id}, 'CONCLUIDA', this)">Concluir</button>
                    </div>
                </div>
            </article>
        `;
    ...
    function protectRenderedImagesV18(container) {
        if (!container) return;
        container.querySelectorAll("img").forEach(function (image) {
            protectImageV18(image, image.classList.contains("reporter-avatar") ? FALLBACK_USER_V18 : FALLBACK_PET_V18);
        });
    }

    window.atualizarChamadoV18 = async function (origin, id, status, button) {
        const card = button ? button.closest(".prof-occurrence-card") : null;
        if (card) {
        ...
        try {
            const user = currentUserV18() || {};
            await apiV18(`/api/chamados/${encodeURIComponent(origin)}/${id}/status`, {
                method: "PATCH",
                body: JSON.stringify({
                    status,
                    funcionarioCpf: cleanCpfV18(user.cpf),
                    observacao: status === "CONCLUIDA"
                        ? "Atendimento concluído pelo profissional."
                        : "Atendimento iniciado pelo profissional."
                })
            });

            queueCacheV18 = null;
            if (status === "CONCLUIDA" && card) card.remove();
            toastV18(status === "CONCLUIDA" ? "✅ Chamado concluído e cidadão notificado." : "🚑 Atendimento iniciado.");

            const active = document.querySelector(".screen.active");
            if (active && active.id === "nearestOccurrenceScreen") await window.abrirOcorrenciaMaisProxima();
            else if (active && active.id === "priorityQueueScreen") await window.abrirFilaPrioridade();
            else await window.abrirOcorrenciasPro();
        } catch (error) {
            if (card) {
                card.style.opacity = "";
    ...
    window.marcarEmAtendimento = function (origin, id) {
        return window.atualizarChamadoV18(origin, id, "EM_ATENDIMENTO", null);
    };
    window.despacharEquipe = function (origin, id) {
        return window.atualizarChamadoV18(origin, id, "CONCLUIDA", null);
    };
    ...
    window.abrirOcorrenciasPro = async function () {
        const container = byIdV18("listaIntegradaPro");
        if (!container) return;
        container.innerHTML = '<div class="occurrence-card"><p>Carregando chamados...</p></div>';
        showScreenV18("proListScreen");

        try {
            const items = await fetchQueueV18(true);
            container.innerHTML = items.length
                ? items.map(function (item) { return callCardV18(item, true); }).join("")
                : '<div class="occurrence-card"><h4>Nenhum chamado pendente</h4><p>Os novos chamados aparecerão aqui.</p></div>';
            protectRenderedImagesV18(container);
            updateProfessionalStatsV18(items);
        } catch (error) {
            container.innerHTML = `<div class="occurrence-card"><h4>Erro</h4><p>${escapeV18(error.message)}</p></div>`;
        }
    };

    function updateProfessionalStatsV18(items) {
        const list = Array.isArray(items) ? items : [];
        const total = byIdV18("statTotal");
        const anon = byIdV18("statAnon");
        const emergency = byIdV18("statEmergency");
        if (total) total.textContent = String(list.length);
        if (anon) anon.textContent = String(list.filter(function (x) { return x.origem === "anonima" || x.anonima; }).length);
        if (emergency) emergency.textContent = String(list.filter(function (x) { return ["ALTA", "CRITICA"].includes(String(x.prioridade).toUpperCase()); }).length);
    }

    window.abrirOcorrenciaMaisProxima = async function () {
        const box = byIdV18("nearestOccurrenceBox");
        if (!box) return;
        showScreenV18("nearestOccurrenceScreen");
        box.innerHTML = '<div class="occurrence-card"><p>Calculando ocorrência...</p></div>';

        try {
            const items = await fetchQueueV18();
            const item = items[0];
            box.innerHTML = item
                ? callCardV18(item, false)
                : '<div class="occurrence-card"><h4>Nenhuma ocorrência pendente</h4></div>';
            protectRenderedImagesV18(box);
        } catch (error) {
            box.innerHTML = `<div class="occurrence-card"><p>${escapeV18(error.message)}</p></div>`;
        }
    };

    window.abrirFilaPrioridade = async function () {
        const container = byIdV18("priorityQueueList");
        if (!container) return;
        showScreenV18("priorityQueueScreen");
        container.innerHTML = '<div class="occurrence-card"><p>Organizando prioridades...</p></div>';

        try {
            const weights = { CRITICA: 4, ALTA: 3, NORMAL: 2, BAIXA: 1 };
            const items = (await fetchQueueV18()).slice().sort(function (a, b) {
                return (weights[String(b.prioridade).toUpperCase()] || 0) - (weights[String(a.prioridade).toUpperCase()] || 0);
            });
            container.innerHTML = items.length
                ? items.map(function (item) { return callCardV18(item, true); }).join("")
                : '<div class="occurrence-card"><h4>Fila vazia</h4></div>';
            ...
            container.innerHTML = `<div class="occurrence-card"><p>${escapeV18(error.message)}</p></div>`;
        }
    };

    function registeredPetCardV18(pet) {
        return `
            <article class="safe-life-pet-card">
                <div class="safe-life-pet-top">
                    <img class="safe-life-pet-photo" src="${escapeV18(petPhotoV18(pet))}" alt="Foto de ${escapeV18(pet.nome)}">
                    <div class="safe-life-pet-info">
                        <h4>${escapeV18(pet.nome || "Pet")}</h4>
                        <small>${escapeV18(pet.especie || "Animal")} • ${escapeV18(pet.raca || "SRD")}</small>
                        <div class="safe-life-normal-badge">CADASTRADO</div>
                    </div>
                </div>
                <div class="safe-life-pet-lines">
                    <div class="safe-life-pet-line"><strong>Dono:</strong> ${escapeV18(pet.nome_dono || pet.dono_nome || "Não informado")}</div>
                    <div class="safe-life-pet-line"><strong>Telefone:</strong> ${escapeV18(formatPhoneV18(pet.telefone_dono || pet.dono_telefone || ""))}</div>
                    <div class="safe-life-pet-line"><strong>Endereço:</strong> ${escapeV18(pet.localizacao || "Não informado")}</div>
                </div>
            </article>
        `;
    }

    window.abrirPetsCadastradosPro = async function () {
        const container = byIdV18("proPetsList") || byIdV18("professionalPetsList") || byIdV18("activeAgentsList");
        try {
            const pets = await apiV18("/api/pro/pets/cadastrados");
            if (container) {
                container.innerHTML = Array.isArray(pets) && pets.length
                    ? '<div class="safe-life-pet-grid">' + pets.map(registeredPetCardV18).join("") + '</div>'
                    : '<div class="occurrence-card"><h4>Nenhum pet cadastrado</h4><p>Somente pets reais e com status cadastrado aparecem aqui.</p></div>';
                protectRenderedImagesV18(container);
            }

            if (byIdV18("proPetsScreen")) showScreenV18("proPetsScreen");
            else showScreenV18("activeAgentsScreen");
        } catch (error) {
            alert(error.message);
        }
    };

    function missingPetCardV18(pet) {
        return `
            <article class="safe-life-pet-card missing missing-pet-pro-card">
                <div class="safe-life-pet-top">
                    <img class="safe-life-pet-photo" src="${escapeV18(petPhotoV18(pet))}" alt="Foto de ${escapeV18(pet.nome)}">
                    <div class="safe-life-pet-info">
                        <h4>🚨 ${escapeV18(pet.nome || "Pet")}</h4>
                        <small>${escapeV18(pet.especie || "Animal")} • ${escapeV18(pet.raca || "SRD")}</small>
                        <div class="safe-life-alert-badge">DESAPARECIDO</div>
                    </div>
                </div>
                <div class="safe-life-pet-lines">
                    <div class="safe-life-pet-line"><strong>Dono:</strong> ${escapeV18(pet.nome_dono || "Não informado")}</div>
                    <div class="safe-life-pet-line"><strong>Telefone:</strong> ${escapeV18(formatPhoneV18(pet.telefone_dono || ""))}</div>
                    <div class="safe-life-pet-line"><strong>Último local visto:</strong> ${escapeV18(pet.local_desaparecimento || "Não informado")}</div>
                    <div class="safe-life-pet-line"><strong>Detalhes:</strong> ${escapeV18(pet.detalhes_desaparecimento || "Sem detalhes")}</div>
                </div>
                <div class="safe-life-pet-actions">
                    <button class="btn" type="button" onclick="abrirConclusaoResgatePetV18(${Number(pet.id)})">Encontrei este pet</button>
                </div>
            </article>
        `;
    }

    window.abrirPetsDesaparecidosPro = async function () {
        const container = byIdV18("activeAgentsList");
        if (!container) return;
        showScreenV18("activeAgentsScreen");
        container.innerHTML = '<div class="occurrence-card"><p>Carregando pets desaparecidos...</p></div>';

        try {
            const pets = await apiV18("/api/pro/pets/desaparecidos");
            missingPetsCacheV18 = Array.isArray(pets) ? pets : [];
            container.innerHTML = missingPetsCacheV18.length
                ? '<div class="safe-life-pet-grid">' + missingPetsCacheV18.map(missingPetCardV18).join("") + '</div>'
                : '<div class="occurrence-card"><h4>Nenhum pet desaparecido</h4><p>Não existem alertas ativos.</p></div>';
            protectRenderedImagesV18(container);
        } catch (error) {
            container.innerHTML = `<div class="occurrence-card"><p>${escapeV18(error.message)}</p></div>`;
        }
    ...
    window.abrirAgentesAtivos = window.abrirPetsDesaparecidosPro;

    window.abrirConclusaoResgatePetV18 = function (petId) {
        const pet = missingPetsCacheV18.find(function (item) { return Number(item.id) === Number(petId); });
        if (!pet) return alert("Pet não encontrado na lista atual.");

        byIdV18("rescuePetId").value = String(pet.id);
        byIdV18("rescueDestinationType").value = "";
        byIdV18("rescueProfessionalAddress").value = "";
        byIdV18("rescueInstitutionName").value = "";
        byIdV18("rescueInstitutionAddress").value = "";
        byIdV18("rescuePickupInstructions").value = "";
        byIdV18("rescueFoundPhoto").value = "";
        byIdV18("rescuePetSummary").innerHTML = registeredPetCardV18({
            ...pet,
            nome_dono: pet.nome_dono,
            telefone_dono: pet.telefone_dono
        });
        window.alternarDestinoResgatePet();
        showScreenV18("petRescueCompletionScreen");
    };

    window.alternarDestinoResgatePet = function () {
        const type = valueV18("rescueDestinationType");
        const pro = byIdV18("rescueProfessionalDestination");
        const institution = byIdV18("rescueInstitutionDestination");
        if (pro) pro.classList.toggle("hidden", type !== "PROFISSIONAL");
        if (institution) institution.classList.toggle("hidden", type !== "INSTITUICAO");
    };

    window.concluirResgatePet = async function (event) {
        event.preventDefault();
        const button = event.submitter || event.target.querySelector('button[type="submit"]');
        const petId = Number(valueV18("rescuePetId"));
        const type = valueV18("rescueDestinationType");
        const user = currentUserV18() || {};
        const photo = await fileToDataUrlV18("rescueFoundPhoto").catch(function () { return ""; });
        const destinationName = type === "INSTITUICAO" ? valueV18("rescueInstitutionName") : user.nome;
        const destinationAddress = type === "INSTITUICAO" ? valueV18("rescueInstitutionAddress") : valueV18("rescueProfessionalAddress");
        const instructions = valueV18("rescuePickupInstructions");

        if (!photo || !type || !destinationAddress || !instructions || (type === "INSTITUICAO" && !destinationName)) {
            alert("Envie a foto atual e preencha todos os dados do local de retirada.");
            return;
        }

        try {
            setBusyV18(button, true, "Concluindo resgate...");
            await apiV18(`/api/pro/pets/${petId}/concluir-resgate`, {
                method: "POST",
                body: JSON.stringify({
                    funcionarioCpf: cleanCpfV18(user.cpf),
                    fotoEncontrado: photo,
                    destinoTipo: type,
                    destinoNome: destinationName,
                    destinoEndereco: destinationAddress,
                    instrucoesRetirada: instructions
                })
            });
            toastV18("✅ Pet encontrado e cidadão notificado.");
    ...
    window.openPetForm = function () {
        const form = byIdV18("petForm");
        if (form) form.reset();
        showScreenV18("scrPetForm");
    };

    window.abrirPetDesaparecido = function () {
        const form = byIdV18("missingPetForm");
        if (form) form.reset();
        showScreenV18("scrMissingPetForm");
    };
    ...
    window.registrarPet = async function (event) {
        ...
        const user = currentUserV18();
        const button = event.submitter || event.target.querySelector('button[type="submit"]');
        if (!user) return alert("Entre novamente na sua conta.");

        try {
            setBusyV18(button, true, "Salvando pet...");
            const photo = await fileToDataUrlV18("petPhoto").catch(function () { return ""; });
            await apiV18("/api/pets", {
                method: "POST",
                body: JSON.stringify({
                    donoCpf: cleanCpfV18(user.cpf),
                    nome: valueV18("petName"),
                    idade: Number(valueV18("petAge") || 0),
                    especie: valueV18("petSpecies") || "Animal",
                    raca: valueV18("petBreed") || null,
                    sexo: valueV18("petSex") || "NAO_INFORMADO",
                    cor: valueV18("petColor") || null,
                    peso: valueV18("petWeight") || null,
                    local: valueV18("petLocation") || "Não informado",
                    observacoes: valueV18("petObservations") || null,
                    foto: photo || FALLBACK_PET_V18,
                    desaparecido: false,
                    statusPet: "CADASTRADO"
                })
            });
            event.target.reset();
            toastV18("🐾 Pet cadastrado no banco.");
            const msg = byIdV18("confirmMsg");
            if (msg) msg.textContent = "O pet foi cadastrado com sucesso.";
            showScreenV18("confirmationScreen");
        } catch (error) {
            alert(error.message);
        } finally {
            setBusyV18(button, false);
    ...
    window.registrarPetDesaparecido = async function (event) {
        event.preventDefault();
        const user = currentUserV18();
        const button = event.submitter || event.target.querySelector('button[type="submit"]');
        if (!user) return alert("Entre novamente na sua conta.");
        ...
        try {
            setBusyV18(button, true, "Publicando alerta...");
            const photo = await fileToDataUrlV18("missingPetPhoto");
            if (!photo) throw new Error("Envie uma foto recente do pet.");

            await apiV18("/api/pets", {
                method: "POST",
                body: JSON.stringify({
                    donoCpf: cleanCpfV18(user.cpf),
                    nome: valueV18("missingPetName"),
                    idade: Number(valueV18("missingPetAge") || 0),
                    especie: valueV18("missingPetSpecies") || "Animal",
                    raca: valueV18("missingPetBreed") || null,
                    sexo: valueV18("missingPetSex") || "NAO_INFORMADO",
                    cor: valueV18("missingPetColor") || null,
                    local: "Não informado",
                    observacoes: null,
                    foto: photo,
                    desaparecido: true,
                    statusPet: "DESAPARECIDO",
                    localDesaparecimento: valueV18("missingPetLastSeen"),
                    detalhesDesaparecimento: valueV18("missingPetDetails")
                })
            });

            event.target.reset();
            toastV18("🚨 Alerta enviado aos profissionais.");
            const msg = byIdV18("confirmMsg");
            if (msg) msg.textContent = "O alerta do pet desaparecido foi enviado.";
            showScreenV18("confirmationScreen");
        } catch (error) {
            alert(error.message);
        } finally {
    ...
    function citizenPetCardV18(pet) {
        const missing = pet.desaparecido === true || pet.status_pet === "DESAPARECIDO";
        return `
            <article class="safe-life-pet-card ${missing ? "missing" : ""}">
                <div class="safe-life-pet-top">
                    ...
                    <div class="safe-life-pet-info">
                        <h4>${escapeV18(pet.nome || "Pet")}</h4>
                        <small>${escapeV18(pet.especie || "Animal")} • ${escapeV18(pet.raca || "SRD")}</small>
                        <div class="${missing ? "safe-life-alert-badge" : "safe-life-normal-badge"}">${escapeV18(pet.status_pet || (missing ? "DESAPARECIDO" : "CADASTRADO"))}</div>
                    </div>
                </div>
                <div class="safe-life-pet-line"><strong>${missing ? "Último local visto" : "Endereço"}:</strong> ${escapeV18(missing ? pet.local_desaparecimento : pet.localizacao || "Não informado")}</div>
            </article>
        `;
    }

    function notificationCardV18(notification) {
        const data = notification.dados || {};
        const photo = notification.foto
            ? `<img class="notification-photo" src="${escapeV18(notification.foto)}" alt="Foto da notificação">`
            : "";
        const location = data.destinoEndereco
            ? `<div class="notification-location"><strong>Local para retirada:</strong><br>${escapeV18(data.destinoNome || "Local informado")}<br>${escapeV18(data.destinoEndereco)}${data.instrucoesRetirada ? `<br><br><strong>Instruções:</strong> ${escapeV18(data.instrucoesRetirada)}` : ""}</div>`
            : "";

        return `
            <div class="safe-notification-card">
                <div class="safe-notification-icon">${notification.tipo === "PET_ENCONTRADO" ? "🐾" : "✅"}</div>
                <div>
                    <strong>${escapeV18(notification.title || notification.titulo || "Atualização")}</strong>
                    <p>${escapeV18(notification.message || notification.mensagem || "")}</p>
                    ${photo}
                    ${location}
    ...
    window.renderPerfilCidadao = async function () {
        const user = saveUserV18(currentUserV18());
        if (!user) return showScreenV18("loginScreen");

        const name = byIdV18("citizenProfileName");
        const contact = byIdV18("citizenProfileContact");
        const container = byIdV18("myPetsContainer");
        if (name) name.textContent = user.nome || "Cidadão";
        if (contact) contact.textContent = [user.email, formatPhoneV18(user.telefone)].filter(Boolean).join(" • ");
        forceProfilePhotosV18();
        if (container) container.innerHTML = '<p class="empty-message">Carregando dados...</p>';
        showScreenV18("citizenProfile");

        try {
            const [pets, notifications] = await Promise.all([
                apiV18("/api/pets?donoCpf=" + encodeURIComponent(user.cpf)),
                apiV18("/api/users/" + encodeURIComponent(user.cpf) + "/notifications-v18")
            ]);
            if (container) {
                container.innerHTML = `
                    <div class="safe-life-profile-block">
                        <h4>🔔 Notificações</h4>
                        ${Array.isArray(notifications) && notifications.length ? notifications.map(notificationCardV18).join("") : '<p class="empty-message">Nenhuma notificação.</p>'}
                    </div>
                    <div class="safe-life-profile-block">
                        <h4>🐾 Meus pets</h4>
                        <div class="safe-life-pet-grid">
                            ${Array.isArray(pets) && pets.length ? pets.map(citizenPetCardV18).join("") : '<p class="empty-message">Nenhum pet cadastrado.</p>'}
                        </div>
                    </div>
                `;
                protectRenderedImagesV18(container);
            ...
            if (container) container.innerHTML = `<div class="occurrence-card"><p>${escapeV18(error.message)}</p></div>`;
        }
    };

    function accountStateTextV18(user) {
        if (user.excluidaEm) return "Conta excluída";
        if (user.bloqueadoAte && new Date(user.bloqueadoAte).getTime() > Date.now()) {
            return "Suspensa até " + new Date(user.bloqueadoAte).toLocaleString("pt-BR");
        }
        return user.ativo === false ? "Inativa" : "Ativa";
    }

    function adminUserCardV18(user) {
        const blocked = user.bloqueadoAte && new Date(user.bloqueadoAte).getTime() > Date.now();
        const deleted = Boolean(user.excluidaEm);
        const isMaster = user.cpf === "45317828791";
        return `
            <article class="admin-user-card ${(!user.ativo || deleted) ? "inactive" : ""}">
                <div class="admin-user-top">
                    <img class="admin-user-avatar" src="${escapeV18(userPhotoV18(user))}" alt="Foto de ${escapeV18(user.nome)}">
                    <div class="admin-user-info">
                        <h4>${escapeV18(user.nome || "Usuário")}</h4>
                        <p>${escapeV18(user.email || "Sem e-mail")}</p>
                        <small>CPF: ${escapeV18(user.cpf)} • ${escapeV18(formatPhoneV18(user.telefone))}</small>
                    </div>
                </div>
                <div class="admin-chip-row">
                    <span class="admin-chip">${escapeV18(user.type || "citizen")}</span>
                    <span class="admin-chip ${user.ativo && !deleted ? "green" : "red"}">${escapeV18(accountStateTextV18(user))}</span>
                </div>
                ${blocked ? `<div class="admin-user-suspension"><strong>Motivo:</strong> ${escapeV18(user.motivoBloqueio || "Não informado")}</div>` : ""}
                ${!isMaster && !deleted ? `
                    <div class="admin-actions">
                        ${blocked || user.ativo === false
                            ? `<button class="btn admin-success-btn" type="button" onclick="reativarContaAdminV18('${escapeV18(user.cpf)}')">Reativar</button>`
                            : `<button class="btn admin-danger-btn" type="button" onclick="abrirModalSuspensaoAdmin('${escapeV18(user.cpf)}', '${escapeV18(user.nome)}')">Suspender</button>`}
                        <button class="btn admin-delete-btn" type="button" onclick="excluirContaAdminV18('${escapeV18(user.cpf)}', '${escapeV18(user.nome)}')">Excluir</button>
                    </div>
                ` : ""}
            </article>
        `;
    ...
    window.abrirGerenciarUsuarios = async function () {
        const container = byIdV18("adminUsersList");
        if (container) container.innerHTML = '<div class="occurrence-card"><p>Carregando contas...</p></div>';
        showScreenV18("adminUsersScreen");
        try {
            const users = await apiV18("/api/admin/users");
            if (container) {
                container.innerHTML = Array.isArray(users) && users.length
                    ? users.map(adminUserCardV18).join("")
                    : '<div class="occurrence-card"><p>Nenhuma conta encontrada.</p></div>';
                protectRenderedImagesV18(container);
            }
        } catch (error) {
            if (container) container.innerHTML = `<div class="occurrence-card"><p>${escapeV18(error.message)}</p></div>`;
    ...
    window.abrirModalSuspensaoAdmin = function (cpf, nome) {
        byIdV18("adminSuspendCpf").value = cpf;
        byIdV18("adminSuspendDays").value = "7";
        byIdV18("adminSuspendReason").value = "";
        byIdV18("adminSuspendUserText").textContent = `Suspender ${nome} por quantos dias?`;
        byIdV18("adminSuspendModal").classList.remove("hidden");
    };

    window.fecharModalSuspensaoAdmin = function () {
        byIdV18("adminSuspendModal").classList.add("hidden");
    };

    window.confirmarSuspensaoAdmin = async function () {
        const cpf = valueV18("adminSuspendCpf");
        const dias = Number(valueV18("adminSuspendDays"));
        const motivo = valueV18("adminSuspendReason");
        if (!dias || dias < 1 || !motivo) return alert("Informe os dias e o motivo.");

        try {
            await apiV18(`/api/admin/accounts/${cpf}/suspend`, {
                method: "PATCH",
                body: JSON.stringify({ dias, motivo })
            });
            window.fecharModalSuspensaoAdmin();
            toastV18("⏳ Conta suspensa e sessões encerradas.");
    ...
    window.reativarContaAdminV18 = async function (cpf) {
        try {
            await apiV18(`/api/admin/accounts/${cpf}/reactivate`, { method: "PATCH", body: "{}" });
            toastV18("✅ Conta reativada.");
            await window.abrirGerenciarUsuarios();
        } catch (error) {
            alert(error.message);
        }
    ...
    window.excluirContaAdminV18 = async function (cpf, nome) {
        if (!confirm(`Excluir a conta de ${nome}? A sessão aberta será encerrada.`)) return;
        const motivo = prompt("Motivo da exclusão:", "Conta excluída pelo administrador.") || "Conta excluída pelo administrador.";
        try {
            await apiV18(`/api/admin/accounts/${cpf}/delete`, {
                method: "DELETE",
                body: JSON.stringify({ motivo })
            });
            toastV18("🗑️ Conta excluída e sessões revogadas.");
            await window.abrirGerenciarUsuarios();
        } catch (error) {
            alert(error.message);
        }
    ...
    window.cadastrarEmpresaAdmin = async function () {
        const payload = {
            nome: valueV18("adminCompanyName"),
            tipo: valueV18("adminCompanyType") || "Empresa parceira",
            cnpj: valueV18("adminCompanyCnpj"),
            telefone: formatPhoneV18(valueV18("adminCompanyPhone")),
            email: valueV18("adminCompanyEmail"),
            endereco: valueV18("adminCompanyAddress")
        };
        if (!payload.nome) return alert("Informe o nome da empresa.");
        try {
            await apiV18("/api/admin/empresas", { method: "POST", body: JSON.stringify(payload) });
            toastV18("🏢 Empresa cadastrada.");
    ...
    function companyCardV18(company) {
        return `
            <article class="company-card ${company.ativo === false ? "inactive" : ""}">
                <div class="company-top">
                    <div class="company-icon">🏢</div>
                    <div class="company-info"><h4>${escapeV18(company.nome)}</h4><p>${escapeV18(company.tipo || "Empresa")}</p><small>${escapeV18(formatPhoneV18(company.telefone || ""))}</small></div>
                </div>
                <div class="admin-actions">
                    <button class="btn ${company.ativo === false ? "admin-success-btn" : "admin-danger-btn"}" onclick="alterarStatusEmpresaAdminV18(${Number(company.id)}, ${company.ativo === false ? "true" : "false"})">${company.ativo === false ? "Ativar" : "Desativar"}</button>
                    <button class="btn admin-delete-btn" onclick="excluirEmpresaAdminV18(${Number(company.id)})">Excluir</button>
                </div>
            </article>
        `;
    }

    window.abrirEmpresasAdmin = async function () {
        const container = byIdV18("adminCompaniesList");
        showScreenV18("adminCompaniesScreen");
        if (container) container.innerHTML = '<div class="occurrence-card"><p>Carregando empresas...</p></div>';
        try {
            const companies = await apiV18("/api/empresas");
            if (container) container.innerHTML = companies.length ? companies.map(companyCardV18).join("") : '<div class="occurrence-card"><p>Nenhuma empresa.</p></div>';
        } catch (error) {
            if (container) container.innerHTML = `<div class="occurrence-card"><p>${escapeV18(error.message)}</p></div>`;
        }
    };

    window.alterarStatusEmpresaAdminV18 = async function (id, ativo) {
        try {
            await apiV18(`/api/admin/empresas/${id}/status`, { method: "PATCH", body: JSON.stringify({ ativo }) });
            await window.abrirEmpresasAdmin();
        } catch (error) { alert(error.message); }
    };

    window.excluirEmpresaAdminV18 = async function (id) {
        if (!confirm("Excluir esta empresa?")) return;
        try {
            await apiV18(`/api/admin/empresas/${id}`, { method: "DELETE" });
            await window.abrirEmpresasAdmin();
        } catch (error) { alert(error.message); }
    };

    async function loadCompanySelectV18() {
        try {
            const companies = await apiV18("/api/empresas");
            ["regCompany", "loginCompany", "adminProCompany", "editProCompany"].forEach(function (id) {
                const select = byIdV18(id);
                if (!select) return;
                const current = select.value;
                select.innerHTML = companies.filter(function (x) { return x.ativo !== false; }).map(function (x) {
                    return `<option value="${escapeV18(x.nome)}">${escapeV18(x.nome)}</option>`;
                }).join("");
    ...
    window.cadastrarProfissionalAdmin = async function () {
        const senha = valueV18("adminProPassword");
        if (!senha || senha !== valueV18("adminProPasswordConfirm")) return alert("Confira a senha inicial.");
        const payload = {
            nome: valueV18("adminProName"),
            cpf: cleanCpfV18(valueV18("adminProCpf")),
            email: valueV18("adminProEmail"),
            telefone: formatPhoneV18(valueV18("adminProPhone")),
            senha,
            company: valueV18("adminProCompany"),
            profissional: {
                cargo: valueV18("adminProRole"),
                especialidade: valueV18("adminProSpecialty"),
                regiaoAtendimento: valueV18("adminProRegion"),
                statusPlantao: valueV18("adminProShiftStatus"),
                veiculo: valueV18("adminProVehicle"),
                equipe: valueV18("adminProTeam")
            }
        };
        try {
            await apiV18("/api/admin/profissionais", { method: "POST", body: JSON.stringify(payload) });
            toastV18("👷 Profissional cadastrado.");
            await window.abrirGerenciarUsuarios();
        } catch (error) { alert(error.message); }
    };
    ...
    window.inicializarPainelPro = async function () {
        const user = saveUserV18(currentUserV18());
        if (!user) return showScreenV18("loginScreen");
        const name = byIdV18("proWelcomeName");
        const company = byIdV18("proCompanyName");
        if (name) name.textContent = user.nome || "Profissional";
        if (company) company.textContent = "🏢 " + (user.company || "Safe Life Matriz");
        forceProfilePhotosV18();
        try { updateProfessionalStatsV18(await fetchQueueV18(true)); } catch (_) {}
        showScreenV18("proDashboard");
    };

    window.inicializarPainelAdmin = async function () {
        const user = saveUserV18(currentUserV18());
        if (!user) return showScreenV18("loginScreen");
        const name = byIdV18("adminWelcomeName");
        if (name) name.textContent = user.nome || "Administrador";
        forceProfilePhotosV18();
        try {
            const summary = await apiV18("/api/dashboard/resumo");
            const map = {
                adminStatUsers: summary.usuarios,
                adminStatProfessionals: summary.funcionarios,
                adminStatReports: (summary.ocorrencias || 0) + (summary.denunciasAnonimas || 0),
                adminStatSuspicious: 0
            };
            Object.entries(map).forEach(function ([id, value]) {
                const node = byIdV18(id); if (node) node.textContent = String(value || 0);
            });
        } catch (_) {}
        showScreenV18("adminDashboard");
    };

    window.logout = function () {
        clearSessionV18();
        toastV18("Sessão encerrada.");
        showScreenV18("loginScreen");
    };

    function bootV18() {
        localStorage.setItem("safeLifeApiUrl", API_V18);
        window.SAFE_LIFE_API_URL = API_V18;
        applyPhoneMasksV18();
        forceProfilePhotosV18();
        loadCompanySelectV18();
        checkSessionV18();

        if (sessionTimerV18) clearInterval(sessionTimerV18);
        sessionTimerV18 = setInterval(checkSessionV18, 12000);
